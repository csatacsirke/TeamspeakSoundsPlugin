...
BOOL CDlgScrollable::OnInitDialog() 
{
	...
	//m_hCursor1 = LoadCursor(AfxGetInstanceHandle(), IDC_ARROW);
	//m_hCursor2 = LoadCursor(AfxGetInstanceHandle(), IDC_ARROW);

	//m_hCursor1 = AfxGetApp()->LoadCursor(IDC_ARROW);
	//m_hCursor2 = AfxGetApp()->LoadCursor(IDC_ARROW);
...
void CDlgScrollable::UpdateScrollView() {

	SCROLLINFO si;
	si.cbSize = sizeof(SCROLLINFO);
	si.fMask = SIF_ALL;
	si.nMin = 0;
	//si.nMax = m_rcOriginalRect.Height() + 100;
	si.nMax = CalculateBottomPos();
...
afx_msg void CDlgScrollable::OnParentNotify(UINT message, LPARAM lParam) {
	if(message == WM_CREATE) {
		OnNewChild((HWND)lParam);
	}
}

void CDlgScrollable::OnNewChild(HWND child) {
	 
}


void CDlgScrollable::AddChild(CWnd* window) {
	CRect rect;
	
	window->GetClientRect(rect);
	int bottomPos = CalculateBottomPos();
	rect += CPoint(0, bottomPos + 20);
...
int CDlgScrollable::CalculateBottomPos() {
	int max = 0;
	int min = 0;
	for(CWnd *pWnd = GetWindow(GW_CHILD); pWnd != NULL; pWnd = pWnd->GetNextWindow(GW_HWNDNEXT)) {
		...
		min = min(rect.top, min);
		max = max(rect.bottom, max);
	}
	return max - min;
}